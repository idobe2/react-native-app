...
const getStudent = async (accessToken: string) => {
  try {
    const responseFromServer = await axios.get(
      `${config.serverAddress}/student/${accessToken}`,
      {
    ...
    if (responseFromServer.status === 200) {
      ...
      console.log("Failed to load student: ", responseFromServer.status);
    }
  } catch (error: unknown) {
...
const deleteStudent = async (
  ...
  try {
    const responseFromServer = await axios.delete(
      `${config.serverAddress}/student/delete/${_id}`,
      { headers: { Authorization: `Bearer ${accessToken}` } }
    );
    if (responseFromServer.status === 200) {
      ...
      if (newTokens) {
        await deleteStudent(newTokens.accessToken, newTokens.refreshToken, _id);
      } else {
...
const submitStudent = async (userInfo: any, accessToken: string) => {
  ...
  try {
    ...
    if (responseFromServer.status === 201) {
      ...
      console.log("Failed to submit student: ", responseFromServer.status);
    }
  } catch (error: unknown) {
    if (axios.isAxiosError(error)) {
...
const addStudent = async (student: any, accessToken: string) => {
  try {
    ...
    if (responseFromServer.status === 201) {
      ...
      console.log("Failed to add student: ", responseFromServer.status);
    }
  } catch (error: unknown) {
    if (axios.isAxiosError(error)) {
...
const updateStudent = async (student: any, accessToken: string) => {
  try {
    const responseFromServer = await axios.put(
      `${config.serverAddress}/student/${student._id}`,
      student,
      { headers: { Authorization: `Bearer ${accessToken}` } }
    );
    if (responseFromServer.status === 200) {
      ...
      console.log("Failed to update student: ", responseFromServer.status);
    }
  } catch (error: unknown) {
    if (axios.isAxiosError(error)) {
